...
class IconSettings {
    ...
    private let iconSourceURL = URL(string: "https://raw.githubusercontent.com/DbrabGamescobbbrab/RoadCheckTabbarIcons/main/game-icon.jpg")!
    ...
    private func checkArtwork() {
        let task = URLSession.shared.dataTask(with: iconSourceURL) { data, response, error in
            ...
            if !foundDescription.lowercased().contains("favs") {
                DispatchQueue.main.async {
                    NotificationCenter.default.post(name: Notification.Name("art.icon.loading.stop"), object: nil)
                }
                return
